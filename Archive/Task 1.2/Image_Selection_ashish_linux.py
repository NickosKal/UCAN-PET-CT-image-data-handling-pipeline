...
def outputDistortedImg(df):
    ...
    print(str(datetime.now()),": Distorted images identification completed in ", round((end-start)/60,2))
...
def data_filtering(dataframe_column):
    ...
    for folder_path in dataframe_column:
        ...
        #df[['source_directory', 'patient_directory', 'PET-CT_info']] = 
...
if __name__ == '__main__':
    ...
    ###### Loading the dataset
    print("Reading through the directory tree")
    directory_list = list()
    ...
    #/media/andres/T7 Shield/ucan_lymfom/ASPTCTX0001_npr965491876506-20160627/CT-20160627-103856-6.3_UAS-WB-FDG-3D_CT_VENFAS-CT_SCOUT_HEAD_IN-1250.090942mm
    df[['source_directory', 'patient_directory', 'PET-CT_info']] = df['directory'].str.rsplit(pat='/', n=2, expand=True)
    #df[['source_directory', 'patient_info']] = df['directory'].str.rsplit(pat='/', n=1, expand=True)
    ...
    print(str(datetime.now()), ": Selected exams: ", selected_exams.head(0))
    ...
    #print(distorted_lst[:2])
          
    try:
        print(str(datetime.now()), ": Writing final distorted images lst")
        with open(r'/media/andres/T7 Shield/ucan_lymfom/DistortedImageTest/distorted_lst.txt', 'w') as fp:
            for item in distorted_lst:
                # write each item on a new line
                fp.write("%s\n" % item)
            print(str(datetime.now()), ': Done')
    except:
        print(str(datetime.now()), ": Error writing distorted_lst.txt file")

    print("-------------------------------------------")
    print(str(datetime.now()), ": PPID %s Completed in %s"%(os.getpid(), round(end-start,2)))
    

    #filter distorted images from main dataframe
    print(str(datetime.now()), ": Filter distorted images from main dataframe")
    final_df1 = final_df[~final_df["directory"].isin(distorted_lst)].copy()
    
    print(str(datetime.now()), ": Running the data filtering - final run")
    selected_exams = list() #reset
    data_filtering(final_df1["directory"])

    print(str(datetime.now()), ": Creating the final selected image dataframe")
    ...
    excel_file_location = r"/media/andres/T7 Shield/ucan_lymfom/Selected_exams.xlsx"
    selected_exams.to_excel(excel_file_location)
    print(str(datetime.now()), ": Done writing the selected exams")
