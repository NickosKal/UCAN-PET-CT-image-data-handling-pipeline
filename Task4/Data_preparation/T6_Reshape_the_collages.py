...
for row, image in df_of_raw_collages.iterrows():

    save_path_temp = os.path.join(reshaped_collages_path, str(image["patient_ID"]), str(image["scan_date"]))
...
df_of_reshaped_collages = pd.DataFrame(columns=["patient_ID", "scan_date", "SUV_MIP", "SUV_bone", "SUV_lean", "SUV_adipose", "SUV_air"])
for patient_ID in tqdm(sorted(os.listdir(reshaped_collages_path))):
    for scan_date in sorted(os.listdir(os.path.join(reshaped_collages_path, patient_ID))):
        for angle in ["-90.0", "0.0"]:
            SUV_MIP_path = os.path.join(reshaped_collages_path, patient_ID, scan_date, "SUV_MIP" + ".npy")
            SUV_bone_path = os.path.join(reshaped_collages_path, patient_ID, scan_date, "SUV_bone" + ".npy")
            SUV_lean_path = os.path.join(reshaped_collages_path, patient_ID, scan_date, "SUV_lean" + ".npy")
            SUV_adipose_path = os.path.join(reshaped_collages_path, patient_ID, scan_date, "SUV_adipose" + ".npy")
            SUV_air_path = os.path.join(reshaped_collages_path, patient_ID, scan_date, "SUV_air" + ".npy")
            ...
            df_of_reshaped_collages = pd.concat([df_of_reshaped_collages, df_temp], ignore_index=True)

df_of_reshaped_collages["CT_MIP"] = df_of_reshaped_collages["SUV_MIP"]
df_of_reshaped_collages["CT_bone"] = df_of_reshaped_collages["SUV_bone"]
df_of_reshaped_collages["CT_lean"] = df_of_reshaped_collages["SUV_lean"]
df_of_reshaped_collages["CT_adipose"] = df_of_reshaped_collages["SUV_adipose"]
df_of_reshaped_collages["CT_air"] = df_of_reshaped_collages["SUV_air"]

df_of_reshaped_collages["CT_MIP"] = df_of_reshaped_collages["CT_MIP"].str.replace("SUV_MIP", "CT_MIP")
df_of_reshaped_collages["CT_bone"] = df_of_reshaped_collages["CT_bone"].str.replace("SUV_bone", "CT_bone")
df_of_reshaped_collages["CT_lean"] = df_of_reshaped_collages["CT_lean"].str.replace("SUV_lean", "CT_lean")
df_of_reshaped_collages["CT_adipose"] = df_of_reshaped_collages["CT_adipose"].str.replace("SUV_adipose", "CT_adipose")
df_of_reshaped_collages["CT_air"] = df_of_reshaped_collages["CT_air"].str.replace("SUV_air", "CT_air")
